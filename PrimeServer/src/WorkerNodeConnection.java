...
public class WorkerNodeConnection implements Runnable {
	...
	int length = 10;
		...
		//	ByteArrayInputStream bais = new ByteArrayInputStream(buf);
		 ...
		 byte[] buffer = "Done".getBytes();
			...
			DatagramPacket sendpacket = new DatagramPacket(buffer, buffer.length, receivedpacket.getAddress(), workerNodeport);
	...
	/*
		try {
			...
			while(true) {
			Socket newCo = socket.accept();
			FarmSocket farmSocket = new FarmSocket(newCo);
			...
			port = Integer.parseInt(data);
				...
				System.out.println("Nt able to parse ");
			}
			FarmId farmId = new FarmId(newCo.getInetAddress(), port);
			FarmProvider.listoffarms.add(farmId);
			System.out.println("new worker Nodes added ");
			newCo.close();
			}
